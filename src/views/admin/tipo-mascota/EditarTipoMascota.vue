...
onMounted(async () => {
  try {
    const { data } = await tipoMascotaService.mostrar(id);
    nombre.value = data.nombreTipoMascota || '';
    descripcion.value = data.descripcion || '';
    activo.value = data.activo !== false; 
  } catch (error) {
    showToast("ERROR", "No se pudo cargar el registro", "error");
...
async function guardarCambios() {
  ...
  try {
    ...
    await tipoMascotaService.modificar(id, payload);
    showToast("¡ACTUALIZADO!", "Cambios guardados correctamente", "success");
    setTimeout(() => router.push("/admin/tipos-mascotas"), 1500);
  } catch (error) {
    console.error(error);
  ...
  <div class="container mt-4">
    ...
    <div class="card shadow-lg border-0" style="border-radius: 15px; overflow: hidden; max-width: 600px; margin: 0 auto;">
      <div class="card-header text-white text-center py-4" style="background: linear-gradient(135deg, #1a6b56, #114539);">
        <h3 class="mb-0 fw-bold"><i class="bi bi-pencil-square me-2"></i> Editar Tipo</h3>
      </div>
      ...
      <div class="card-body p-4 p-lg-5 bg-light">
        <form @submit.prevent="guardarCambios">
          <div class="mb-4">
            ...
            <input type="text" class="form-control form-control-lg shadow-sm" v-model="nombre" required>
          </div>

          <div class="mb-4">
            <label class="form-label fw-bold text-success">Descripción</label>
            <textarea class="form-control shadow-sm" rows="3" v-model="descripcion"></textarea>
          </div>
