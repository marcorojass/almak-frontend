...
onMounted(async () => {
  try {
    const { data } = await metodoPagoService.mostrar(id);
    nombre.value = data.nombreMetodoPago || '';
    descripcion.value = data.descripcion || '';
  } catch (error) {
    showToast("ERROR", "No se pudo cargar el registro", "error");
    router.push("/admin/metodos-pagos");
  }
});
...
async function guardarCambios() {
  ...
  try {
    ...
    await metodoPagoService.modificar(id, payload);
    showToast("¡ACTUALIZADO!", "Cambios guardados", "success");
  ...
  <div class="container mt-4">
    ...
    <div class="card shadow-lg border-0" style="border-radius: 15px; overflow: hidden; max-width: 600px; margin: 0 auto;">
      <div class="card-header text-white text-center py-4" style="background: linear-gradient(135deg, #1a6b56, #114539);">
        <h3 class="mb-0 fw-bold"><i class="bi bi-pencil-square me-2"></i> Editar Método</h3>
      </div>
